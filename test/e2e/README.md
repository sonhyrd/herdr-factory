...
| `comment-brand-default` | the control for the same: with no `source_comments` and no `host_alias`, the ledger lines are byte-identical to the pre-brand factory (`[herdr-factory claim id=<run> host=<hostname>]` + its release) |
