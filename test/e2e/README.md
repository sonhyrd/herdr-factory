...
| `idle-nudge-unknown` | issue #112 *(fake lane, `HF_AGENT_SILENT`)*: an `unknown`-status pane whose output keeps changing is never nudged; once its screen holds still for the window it is nudged on the confirmed path followed by `agent send-keys <pane> enter`, and again at 2× with the exact step-done command — two at most |
| `idle-nudge-budget` | issue #112: a step whose budget expires while `working` and then idles without signalling is not parked on its first idle tick — two nudges (the second with the step-done command), then the `step_budget` park |
| `layout-undetected-agent` | issue #99 *(fake lane — `HF_HERDR_UNDETECTED` lists a live agent `unknown` with no `agent` label, which a real herdr cannot be asked for)*: the layout wait relaunches such a pane exactly once — `pane run /quit`, then the layout's `cursor-agent` command, never `agent start` — records `pane_relaunched {detected: true}`, and the next dispatch lands with no park; an agent that stays undetected gets its one relaunch and then parks `<step>: agent in pane <id> was never detected by Herdr (status unknown)`, which `explain` repeats without blaming plugin links or tab names |
...
| `dirty-tree-guard` | issue #66: a step that never commits is pinned to its tree. The work agent leaves an uncommitted edit behind, so the advance into the gate parks `dirty_tree` with the diff stat and the gate is never dispatched; once the tree is cleaned and the run resumed, the gate starts — and when the gate itself edits without committing, its own `step-done` exits 1 with the stat and the belt does not advance |
| `work-item-edited` | issue #97: a Jira ticket edited after the claim. With three tickets on one `JiraFake` board, two get a new description while work runs: each parks `work_item_edited` (`ticket edited after claim: Description`) at the advance into review, review is never dispatched, `ticket.json` is refreshed (`ticket.claimed.json` keeps the claim-time copy), `work-item-edits.md` holds the before/after, and a note with both choices lands on the ticket. `resume` ships the first as is (no second park); `rework … work` re-runs work on the second (pass 2), which then reaches review. The third only gains a label (`updated` moves, content doesn't) and never parks |
| `memory-exclude` | issue #110: in a target repo whose committed `.gitignore` never mentions `.memory/`, two runs — each with its work doc materialized into `.memory/herdr-factory/` — pass the read-only `review` gate and complete with no `dirty_tree` park, because every worktree create appends `.memory/` to the checkout's shared `info/exclude`; that file ends with exactly one `.memory/` line and the tenant's `.gitignore` is untouched |
| `gate-receipts` | issue #68: the work step's verification commands run through the rendered `@@GATE_CMD@@` and leave one receipt per (gate, SHA) — a gate re-run at the same commit replaces its receipt rather than adding one — the review step reads them (`CURRENT` against this HEAD) instead of re-running anything, `step-done` prints `advanced work → review`, and each pass exports its wall/shell/model split as a `step_timing` event |
| `operator-rework` | issue #66: `rework <KEY> work --note …` typed while the EVIDENCE step is live stops it, opens work's next pass with the note behind the rework banner, records a `rework` event (`by: operator`) that `timeline` prints — and the belt then runs forward and completes, so the rewind is not an abort. Also pins the finish protocol's `sha: <commit>` line in the re-rendered prompt |
...
| `pr-review-watch` | a draft PR keeps the step-done gate, a ready one hands off without it, a new review thread wakes a resolver that holds a slot only while working, and a green PR notifies the operator once per green head (a resolved thread on the same head is not news) — while the factory never merges it |
| `pr-watch-green-once` | issue #114: a red CI flicker on the same head does not re-notify (`/status` still shows it red meanwhile); a green push does, once. A `CONFLICTING` PR never notifies and wakes the resolver once per head (`baseMerge: conflicting`, prompt says merge the base, never the PR); `BEHIND` notifies on a non-strict base and wakes the resolver (`baseMerge: behind`) on a strict one. Never merged |
| `resolver-work-agent` | issue #88 *(real lane)*: the PR-watch resolver runs on the WORK step's agent. With work = `claude --hf-work-b` and pr = `opencode --hf-pr-a`, a review thread re-prompts the live work pane (never the pr pane) and `run.pane_id` follows it; with the work pane closed, the next thread's `agent start` carries B's kind and flags, never A's, and the new pane becomes the work step's (session cleared). Both resolving panes are published `hf_state=watching` |
...
| `dev-server-step-change` / `dev-server-park` | issue #98: a listener the WORK AGENT started (detached, cwd in the worktree, port in `<git dir>/hf-port`) is dead — `dev_server_stopped {port, pids, rssKb, why}` recorded — once the run is on the next step, and once it parks on `ask-human`, with the worktree still live; a run with **no** `hf-port` reaches the next step and merges with no kill |
| `needs-you` | issue #130: two real servers, fake herdr — `needs-you --line` prints nothing at N = 0, `hf: 1 need you` once a run asks a human, `hf: 3 need you` (two asks + the board's `unverifiable` row) within 10s after the remote stops, and nothing once the local server is down; with `machine.yml` `notify_needs_you: true` each arrival is announced once via `herdr notification show "<repo> <key>"`, and a server restart does not re-announce the one already waiting |
...
| `doctor-live-config` | issue #115: with the world's config dir made a checkout of a bare origin two commits ahead, `doctor --deep` fetches and turns `live config up to date` amber with `2 commit(s) behind origin/main (<oldest subject>)` while the live HEAD stays put (fetch only); a hand fast-forward turns it green; a hand-edited `repos/app/config.yml` is named and an untracked file is not |
| `checkout-sync` | issue #71: `ensure-up` fast-forwards a repo's MAIN checkout. With a real `serve` up, a checkout left BEHIND its bare origin catches up on the next `ensure-up` (still a `noop` — the sweep restarts nothing) and `doctor` names the fast-forward it did; a second sweep inside the 10-minute throttle does nothing; a hand-patched (dirty) tree is skipped with the edit intact and the doctor row amber `dirty tree, skipped`; clearing the tree lets the next sweep catch up on its own |
...
| `eligible-match-filter` | issue #81: what the board may call READY, on a MULTI-REPO host over one Jira query. Two repo configs differing only in their `match` file each list their own tickets and none of the other's; an item whose predicate THROWS is listed by neither and its drop is on the record; 20 refreshes across both repos still cost **0** source calls (the match runs in the poll); and `herdr-factory --repo <r> eligible` agrees key-for-key with that repo's `GET /eligible` — one poll ⇒ one drop line, because the claim reads the verdict rather than re-running it |
...
| `comment-brand` | issue #70: `source_comments.brand` + `machine.yml`'s `host_alias` reach the wire — the claim ledger is written `[hf claim id=<run> host=contabo]` with the machine's real name nowhere on the tracker, a LEGACY `[herdr-factory claim … host=<hostname>]` this host left before the rename is recognised as its own and released BEFORE the fresh claim — **under the hostname the claim carries**, so every other host pairs them too (no self-fence mid-rollout) — and another host's legacy claim still fences the item with nothing posted |
| `comment-brand-default` | the control for the same: with no `source_comments` and no `host_alias`, the ledger lines are byte-identical to the pre-brand factory (`[herdr-factory claim id=<run> host=<hostname>]` + its release) — and the mixed-brand half of a rollout, in the direction only this world can show: an item carrying a FLIPPED host's `[hf claim …]` still fences this un-flipped factory, which has never heard of that brand |
| `comment-brand-legacy-question` | issue #70's human loop across the switch: a question already on the ticket under the OLD marker is adopted by the idempotence scan rather than asked again, a note bearing the OLD marker is still not a human reply (INV-6 reading both brands), a human reply that merely brackets the brand (`see [hf-204] …`) IS taken as the reply, and the NEXT question the factory writes carries the configured brand |
...
| `fleet-forward` | issue #62: the SSH forward ITSELF, under a TUI kept alive across polls. `build-box` is declared `ssh: true`, so it gets no endpoint override and the shipped `SshForwardTransport` really runs over the harness's fake `ssh` (`harness/ssh-fake`): one forward brings a two-repo machine up and is reused across polls, restarting that remote under a live ControlMaster tears the held forward down with `ssh -O cancel -L <its port>` rather than dropping it still running, the board recovers on a fresh port **in the same TUI process**, and the fake ssh's own start/exit log shows never more than one forward open at once |
...
| `tui-fleet-claimed` | issue #81: a ticket is claimed ONCE for the whole fleet. Machine A's `/eligible` still offers an item machine B has claimed (its server cannot know), and the board must not draw it as ready — with an unclaimed control item that IS a ready card in the same frame, so the absence is a real one, and the machine filter unable to take it back |
| `tui-board` | the redesigned board on a MULTI-REPO host: the host gate printed once (never per repo row), repos with nothing in them collapsed to one `idle ·` line that `i` expands, a cause two repos share named once on the machine header while both rows keep their `d` detail, an eligible-but-unclaimed item that never falls into the idle line, a one-run belt drawn as one compact line instead of a column grid — and a machine that stops answering turning its collapsed line `unverified`, never `idle` |
| `tui-open-row` | getting from a row to the thing it is about: `o` on the green needs-you row handing that PR's URL to `$BROWSER` (a recorder script — the container has no display), `O` handing the ISSUE instead, `o` on a belt card with no PR yet falling back to its work item, a `local_markdown` run answering `no PR or work item URL`, the OTHER machine's run opening in THIS machine's browser, and a second TUI with `SSH_CONNECTION` and no display printing the URL on the action line rather than spawning anything |
| `tui-needs-you` | the section the board leads with: absent while only healthy work is on screen, then carrying a green PR waiting to be merged (`/status.prGreen`, straight off the `pr_green` watch), a run parked for attention and a run holding for a human answer — all three above the machine headers |
