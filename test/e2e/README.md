...
| `review-verdict-handoff` | issue #86: factory review verdicts posted as PR review BODIES (no threads) on two green PRs. `changes-requested` round 1 is handed to the resolver (a `review_handoff` event; its prompt carries the findings; `explain` and `/status` say `fixing hf-review round 1`). The fix push re-runs the gates with notes scoped to `git diff H1..H2`, and the scripted review step posts round 2 (`-review-round: 2`, `-review-head: H2`) instead of bouncing. After a second must-fix pass, round 3 posts `needs-human`: a notification, no third hand-off, no ready to merge. On the other run, `unchanged` and nits-only `clean` wake nothing, `clean` with a should-fix does, a docs-only fix still self-checks, and ready to merge fires only after round 2 comes back clean. PR #99, which no run watches, is never queried or written |
