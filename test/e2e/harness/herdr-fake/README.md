...
| `agent send-keys` | `TARGET KEY…` | `enter` writes an empty line to the agent's stdin (a submitted empty input); other keys are accepted and ignored. The engine sends it after nudging an `unknown`-status pane |
...
| — | `undetected: [key…]` | `HF_HERDR_UNDETECTED=<key>[=sticky][,…]` | keyed on the pane's `HERDR_FACTORY_TICKET`: `agent list` shows its LIVE agent `unknown` with **no** `agent` field, and `agent prompt` answers `agent_not_found` — an agent herdr never detected (issue #99). `<key>` holds until the agent is relaunched by typing its command (`pane run`); `<key>=sticky` never lets it be detected. `pane run <id> /quit` into a live agent exits it, and `pane run` of `cursor-agent …` is detected as kind `cursor`. See `layout-undetected-agent` |
