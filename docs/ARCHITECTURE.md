...
| `watchers/needs-you.ts` | The **needs-you notifier**, run by `serve`: every 30 s, when `machine.yml` sets `notify_needs_you`, one `quickView` of the fleet (status phase only) through `needsYou`, diffed against `<state>/needs-you-notified.json` (`diffNeedsYou`). An item is `machine\|repo\|key` — not its reason — and is announced once per entry. Machine-level entries (an unverifiable box) are never announced, and an id whose machine is unverifiable this read is **kept**, so a box going quiet never makes its items "leave" and re-announce. A failed `herdr notification show` leaves the id out so the next poll retries; a headless `{"shown": false}` is exit 0 and counts as sent. The flag is re-read each poll, so it needs no reload. `needs-you --line` (CLI) is the same `quickView` + `needsYou` count (`needsYouLine`), empty unless the local machine answered. |
