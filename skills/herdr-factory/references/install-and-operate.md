...
| **The agent skills your prompts name** | the prompts delegate the real work to skills (`pr-review`, `pw-prove`, `to-spec`, …); a missing one doesn't fail — the agent just improvises | install/link them under the engine's own root: `~/.claude/skills/<name>/SKILL.md` for Claude steps; for Cursor steps any of `~/.cursor/skills/`, `~/.claude/skills/` or `~/.agents/skills/` (Cursor reads all three) | `doctor --repo <r> --deep` → `agent skills`, which reads the names out of the **loaded prompts** and stats each one. `none named by the prompts` when there are none |
