...
| 6 | The agent harness on PATH **and** `herdr integration install <agent>` run on this machine (see [install-and-operate.md](./install-and-operate.md)) | **Nothing in `doctor` checks the integration** | Without the integration hook herdr never reports `idle`/`working` for that harness. Dispatch tolerates that (an `unknown` agent is prompted, and the submission is confirmed by the pane's screen changing), but everything that reads the agent's state degrades: the idle nudge fires only once the pane's screen has held still for the whole window, `resume` declines to re-prompt, and the liveness/stall diagnoses go blind |
...
(`dev_server_stopped` in `timeline`). `doctor --deep` names listeners under `~/.herdr/worktrees`
whose run is finished or parked — report only; it never kills.
