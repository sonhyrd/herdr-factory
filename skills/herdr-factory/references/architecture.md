...
2. **Ready-to-merge notification** (`noteGreenPr`): `green = state OPEN && !isDraft && unresolved === 0 && failing === 0 && pending === 0`. On the first pass that sees a green PR, `deps.herdr.notify("herdr-factory: <KEY> ready to merge", "<KEY> is ready to merge — PR #n \"title\" in <owner/repo>, green for <dur> · <url>")` and record a `pr_green` event. **The factory never merges** — this is a notification only, and no code path anywhere calls `gh pr merge`. Episode state lives in `watch_state` (run, `pull_request`, `pr_green`): `basedAt` = when the current green started, `sig` = the "notified" mark, **both cleared as soon as the PR stops being green**. So: once per green, never once per tick; red → green → red → green is two notifications, not four; a new commit (checks back to pending) ends the episode and the next green notifies again.
