...
The **tree guard** (`core/tree-guard.ts`) shares the `read_only` pin but is not a watch: it is checked synchronously at the two seams where a read-only step's verdict is about to be trusted. `applySignal`'s `step-done` is **refused** (exit 1, with the diff stat, recording `step_done_refused`) when the worktree is DIRTY; `parkIfTreeDirty` parks the run `dirty_tree` before the forward advance into such a step, its spawn, or a `rework` re-dispatch. It says nothing about HEAD on purpose: a commit is the watch's park, which a step-done RESCUES, and refusing that step-done as well would wedge the run (an agent cannot un-commit). The refusal is stamped into the `read_only` watch row's `meta`, which `runObligations`/`explain` surface so an operator can see why a running step never finishes.
