...
| `running` → `attention` | any guard park, `bounce_limit`, `pr_closed`, `work_item_edited`, `unknown_step`, mid-flight `source_item_stale`, `external_wait_deadline` |
...
| `reviewing` → `running` | the PR head moved past the evidence head with a code change — rework through the verification steps + pr step (`rework` event, `by: "pr_watch"`) |
...
The wait window is `limits.layout_wait_seconds` (600). Past it, if the step's `layout_wait` counter is below the respawn limit (3), a workspace with **none** of the layout's tabs gets the **layout built** by the engine itself (the hook built nothing — typically because the repo's config didn't load when the workspace was created); otherwise the layout pane's **own agent is re-started** when that pane resolves and is sitting at a shell prompt with no agent (the same `agent start` the build issues — kind and args off the layout pane, under the name the build gave it, Claude's folder trust pre-answered; the layout hook starts a pane's agent only once, so a start that failed would otherwise never be retried), and the wait is then **re-armed in place** (`layout pane … not up after Ns — re-arming the wait (retry i/limit)`). Only when the budget is spent does it park `layout_wait_timeout` — its reason suffixed `— layout hook: <the hook's failure line for that workspace>` only when the build FAILED (a config that didn't load, an apply error, `engine rebuild failed: …`), which `explain` leads with; a built or deliberately-skipped layout leaves the reason as before. Wall clock before a park: `(1 + 3) × layout_wait_seconds` ≈ 40 min at defaults. The same 3 credits are shared by the in-place re-arm and the post-park rescue; a successful dispatch or a `resume` refunds them. Before any credit is spent, a pane herdr lists `unknown` with **no agent label** (an agent it never detected) is **relaunched once** per step after ~2 min: `/quit`, then the layout's command is typed at the shell (never `agent start`, which herdr's stale registration refuses). This records `pane_relaunched`. If the run still parks, the reason names it: `agent in pane <id> was never detected by Herdr (status unknown)`.
...
0. **Config gate**: the repo's `config.yml` on disk must still load with the running code (a full load, re-done only when the file's mtime changes — the server otherwise keeps its in-memory config). If it doesn't ⇒ problem `config` (`config invalid: <error> — claims paused until the file loads`, shown in the TUI's "needs you" and by `doctor`), log `config invalid: … — not claiming`, `touchTick`, **return**. The first pass whose file loads clears it (`config loads again — claims resume`) and claims as normal — no restart, no `reload`.
...
2. **Ready-to-merge notification** (`noteGreenPr`): `green = prStepDone && state OPEN && !isDraft && unresolved === 0 && failing === 0 && pending === 0 && mergeable !== UNKNOWN && !baseMergeNeeded` (`baseMergeNeeded`: `mergeable === CONFLICTING`, or `mergeStateStatus === BEHIND` on a base whose protection requires up-to-date branches — `strictBase`; a BEHIND PR on a non-strict base is green). `prStepDone` is the gate added for issue #78: `enterReviewing` moves the run to `reviewing` as soon as a review-ready PR is adopted — *before* the pr step's `step-done` — so the agent may still be polling CI and may still push. While that step's `run_steps` row exists and is not `done`, the PR is not green (the mark is cleared), so the first tick after `step-done pr` is the one that notifies. A run adopted into `reviewing` that never spawned the step has no row and is not gated. Since `run.step` is `null` in `reviewing`, `step-done` for the belt's PR-opening step is accepted there as the one exception to the "must name the run's active step" check. On the first pass that sees a green PR, `deps.herdr.notify("herdr-factory: <KEY> ready to merge", "<KEY> is ready to merge — PR #n \"title\" in <owner/repo> is green with no unresolved threads · <url>")` and record a `pr_green` event. **The factory never merges** — this is a notification only, and no code path anywhere calls `gh pr merge`. Episode state is one `watch_state` row (run, `pull_request`, `pr_green`): `sig` is the head that is green **now** (cleared when it stops being green — the dashboard's `prGreen`), `meta` (`{"toldHead": <sha>}`) is **the head commit the operator was told about** (`PrInfo.headOid`, from `headRefOid` on both PR lookups; issue #114). A red flicker keeps `meta`, so the same head never notifies twice; a *different* green head is news — so a push is a new green even on a repo with **no CI**. A lookup without a head SHA marks `"green"` and degrades to once per green episode. No duration is reported (the watch fires on the first tick that sees green, so any "green for …" would be zero). Accepted edge: a pushed commit whose check runs do not exist yet reads as green for a few seconds, so a tick in that window pings early — and since that head is then marked told, its real green stays quiet.
   **Evidence belongs to a head** (issue #84, `core/evidence-head.ts`) — evaluated *before* the notification: the PR head is compared with the pinned `read_only` baseline of the first **verification step** (the contiguous read-only steps right before the PR-opening step). The verdict lives in `watch_state` (run, `pull_request`, `evidence_head`): `sig` = PR head, `meta` = `{evidenceHead, stale, files}`. `stale` = `git diff --name-only` between them names a non-docs file (docs/Markdown/translations are `*.md`, `*.mdx`, `*.rst`, `*.txt`, `*.adoc`, `docs/`, `locales/`, `i18n/`, `translations/`; an un-diffable pair counts as code). Stale ⇒ not green; and once `resolverActive` is false and the pr step's row is `done`, `bounceStep(…, {watch:true})` rewinds from the pr step to the first verification step (phase `running`, `pr_green` cleared, `resolverActive=false`, feedback note naming both SHAs + files + "mark the PR body's evidence superseded", counted toward `max_bounces`, `rework` event `by: "pr_watch"`). The forward pass re-runs the gates and the pr step, which re-adopts the PR into `reviewing`.
3. `actionable = unresolved > 0 || failing > 0 || baseMergeNeeded`; `fresh = actionable && roundSig !== run.lastThreadSig`, where `roundSig` is `sig`, or `<sig>:<conflicting|behind>:<head>` while the base needs merging — so a conflict wakes the resolver once per head (its prompt merges the base; the merge commit then re-runs the gates via the evidence-head rule). `resolver_woken` carries `baseMerge`.
...
3. Remove the worktree — reading `<absolute git dir>/hf-port` **first** (the dev-server port the run reserved; see [target-repo.md](./target-repo.md)): `herdr worktree remove` → `workspace close` if it survives → `rm -rf` the worktree path (guarded so it can never be the repo itself) → `git worktree prune` → `git branch -D` for **every** name the run used (`worktree_name`, the current `branch`, and the HEAD branch read just before the checkout was destroyed), skipping protected ones (`base_ref` / the main checkout's branch). Once the workspace is closed (and before the `rm -rf`) it kills whatever still LISTENs on `hf-port` — `lsof -nP -ti tcp:<port> -sTCP:LISTEN` → only a listener whose cwd (`lsof -a -p <pid> -d cwd -Fn`) is inside the run's worktree (realpath prefix) is touched — any other or unreadable cwd is left running and logged `pid P belongs to <cwd>, not this run; left running` — and its **process group** gets `SIGTERM`, grace, then `SIGKILL`. A dev server started in a layout pane is reparented when the workspace closes, so this is the only thing that reaps it. **Never a pattern kill** (`pkill -f`/`killall` would take out sibling runs' servers), and never fatal: a missing/unreadable `hf-port`, or a port nothing is listening on, is one log line (naming the `lsof` it ran) and teardown carries on; the kill line carries each pid's RSS and cwd. The same kill (`stopDevServer`) also runs at **every step change** (step-done advance or hand-off to the PR watch, bounce, operator rework) and **every park** (attention, waiting_for_human) — the next step starts whatever server it needs — and a kill that found a listener records a `dev_server_stopped` event (`port`, `pids`, `rssKb`, `why`, `step`).
...
**The idle nudge sits in front of all of them** and is not a guard — it never parks. When a running step's watches evaluate to `none` and its pane has been continuously `isReadyForInput` (`idle`/`done`) — or `unknown` with an unchanged herdr pane `revision` — for `limits.idle_nudge_seconds` (default **300**, `0` disables), the engine sends it the same message `resume` sends — "continue the step; if it is already complete, write your handoff note and run the step-done command now" — pointed at `.memory/herdr-factory/prompt-<step>.md` (and `TASKS.md`, if the agent wrote one), whose baked `--pass` stamp is still valid (a nudge never bumps the pass). A second nudge at 2× the window carries the exact step-done command; at most two per idle **episode**, marked in a `watch_state` row (`run`, step, `idle_nudge`): `based_at` = first seen idle, `sig` = the HEAD when the latest nudge went out, `meta` = `{pass, n, at, rev, over, held}`. An `unknown` pane's nudge is confirmed (`--wait`) and followed by one `enter` (Cursor can leave the text typed but unsubmitted). The episode ends — and the row clears — when the pane goes `working` again, when HEAD moves, or on a `resume`. A `working` pane is never nudged; a gone pane belongs to the respawn path. It rides the memoized pane state **and does not wait for confirmation** (`agent prompt` with no `--wait`), so it costs no extra herdr call per tick and never pins the run lock the `step-done` it is eliciting needs — `nudged` in its event means "herdr accepted the submission". Only `resume` (and an `unknown` pane's nudge) waits for the agent to actually wake. A `step_budget`/`step_stalled` window that expired while the agent was `working` (extended) no longer parks on the first idle tick: the park is held for the nudges, at most 3× the window. Each nudge records an `idle_nudge` event (`detail`: `step`, `pane`, `nudged`, `nth`, `worker`, `idleSeconds`), and `explain` reports it in the `step_budget` / `step_stalled` narratives.
...
| `layout-hook/` | the layout hook's own state: `applied/<sha1>` claims (one per worktree), `decided/<workspaceId>` (focus-event cache, cleared at herdr startup), `failed/<workspaceId>` (the hook's failure line for the workspace — written only when a build failed, removed by any other outcome — quoted by a `layout_wait_timeout` park; cleared with `decided/`), `setup/<token>.status` (a build's setup exit code, reaped after a day) |
