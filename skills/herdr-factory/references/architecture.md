...
**Getting out of a backoff you have already fixed.** Only causes the engine can *probe* shorten themselves: a source auth pause (above) and S3 credentials (the `evidence_publish` pre-pass, `auth`-classified rows only). Anything classified `transient` — a timeout, an unknown SDK error, a source that was briefly 500ing — sits out the full curve. `retry-now` (`POST /repos/{repo}/retry-now`, CLI `retry-now [KEY]`, `s` on the TUI board) is the operator's due-now: every backed-off `pending` row in the repo, or one run's, becomes due immediately and Phase 0 is then flushed **under the repo tick lock**, so the rows land on that call. `attempts` is not reset (a still-broken cause backs off to where it was), rows already due are not counted (`requeued: 0` ⇒ the delay is not a backoff), `waiting` rows are untouched, and no run is mutated — which is why it, and not `resume`, is the fix for a healthy run whose only stuck thing is a background upload.
