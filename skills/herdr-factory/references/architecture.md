...
| `layout-hook/` | the layout hook's own state: `applied/<sha1>` claims (one per worktree), `decided/<workspaceId>` (focus-event cache, cleared at herdr startup), `last/<workspaceId>` (the hook's last outcome line for the workspace, quoted by a `layout_wait_timeout` park; cleared with `decided/`), `setup/<token>.status` (a build's setup exit code, reaped after a day) |
