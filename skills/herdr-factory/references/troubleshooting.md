...
| HEAD moved under a read-only step | `the <step> step cannot finish on this tree — HEAD moved from <a> to <b> while this step ran. …` | Same guard, commit side. The pin *tracks* HEAD until this step's agent is first seen `working`, so a prior step's trailing commit is absorbed, not refused |
