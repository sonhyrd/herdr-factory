...
herdr-factory --repo <r> timeline <KEY> | grep -E 'pr_opened|resolver_woken|review_handoff|review_verdict|review_verdict_skipped|pr_green'
```
...
| A factory review verdict on the PR, no `review_handoff` | a `review_verdict_skipped` event (the review's author is not the factory's `gh` login, or its `-review-head:` is not an ancestor of the PR head — a force-push), the verdict is `unchanged` / nits-only `clean` / `needs-human`, the run already used its 2 fix passes (a `review_verdict` event with `phase: needs-human`, and a notification), or the resolver is still `working`. A verdict on an ancestor of the head (a merge of the base) IS handed off | `explain <KEY>` shows the hand-off state (`fixing hf-review round N`, `self-check round N …`) or the ignored-review line. Past 2 passes, or a resolver that stopped without pushing while a must-fix remains, the operator decides |
...
| `agent skills` ✗ (`--deep`) | `<name> missing at ~/.cursor/skills/<name>/SKILL.md` (or `~/.claude/…`) — one entry per missing skill | Install or re-link the skill on that host, under the root of the engine that runs the step. The names are read from the **loaded prompts** (a `~/.claude|.cursor/skills/<name>/…` path, or a bolded `**<name>** skill`), so renaming a skill directory — or a prompt naming a skill nobody installed — is what trips this |
...
| **Skills are only checked when a prompt NAMES them** in one of the two recognised forms — a skill a prompt only alludes to ("use the repo's review skill"), or one an installed skill depends on in turn, is invisible | `ls ~/.cursor/skills ~/.claude/skills`; read the skill's own `SKILL.md` for what it shells out to |
