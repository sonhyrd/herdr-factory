...
| Branch | Signature | Fix |
|---|---|---|
| Nothing actionable | no unresolved threads, no failing checks, no conflict with the base (`BEHIND` counts only on a base that requires up-to-date branches) | Correct. It waits for a human to merge or close — **the factory never merges**. If the checks have all *concluded* green you also got one "ready to merge" notification (`pr_green` in the timeline) |
| Green PR, no "ready to merge" notification | **the pr step has not signalled `step-done` yet** (it opened the PR and is still polling CI/bot reviews — look for `step_done` with `step: pr` in the timeline), a check has not concluded yet (`pending > 0` — "not failing" is not green), the PR is a draft, a review thread is unresolved, the PR conflicts with its base (or is `BEHIND` a base requiring up-to-date branches — the resolver is woken to merge it; `resolver_woken` carries `baseMerge`), GitHub is still computing `mergeable` (`UNKNOWN`), or the operator was already told about **this head** (the mark is the head commit and survives a red flicker; only a new green head is news) | `gh pr checks <n>` for a still-running check; `gh pr view <n> --json headRefOid,mergeable,mergeStateStatus`; the `pr_green` event's `head` for the mark; otherwise nothing to fix — it fires once per green head |
...
operator; no action needed] …` message; only a *work* error (`bounce_limit`, `pr_closed`, `work_item_edited`) posts a
...
| `work_item_edited` | the edit check at the forward advance into a read-only or PR-opening step (`parkIfWorkItemEdited`) — a `jira`/`github_issues` item's description / acceptance criteria (Jira: any rich-text field; GitHub: the body) differs from the copy recorded at claim | `ticket edited after claim: <fields>` | **no** (a note is posted on the item) | The work doc is already refreshed (claim-time copy: `ticket.claimed.json` / `task.claimed.md`) and `.memory/herdr-factory/work-item-edits.md` has the before/after. `rework <KEY> <step> --note …` redoes the work against the new text; `resume` ships as is. Either way the edit is acknowledged and won't re-park. A label/status/assignee change never parks |
...
| `live config up to date` ⚠ | `live config N commit(s) behind origin/main (<subject>)` / `uncommitted changes to <files>` / `could not check — <git error>` | `~/.config/herdr-factory` lags merged config changes or carries hand edits — nothing syncs it. In that dir: `git pull --ff-only`, or commit/discard the edits. Shallow `doctor` compares against the last fetch; `--deep` fetches first. Doctor never pulls |
