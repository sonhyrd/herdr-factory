...
herdr-factory --repo <r> timeline <KEY> | grep -E 'pr_opened|resolver_woken|review_handoff|review_verdict|pr_green'
```
...
| A factory review verdict on the PR, no `review_handoff` | its `-review-head:` is not the PR's current head (a verdict the head moved past is treated as already acted on), the verdict is `unchanged` / nits-only `clean` / `needs-human`, the run already used its 2 fix passes (a `review_verdict` event with `phase: needs-human`, and a notification), or the resolver is still `working` | `explain <KEY>` shows the hand-off state (`fixing hf-review round N`, `self-check round N …`). Past 2 passes the operator decides — fix by hand, or re-label for a fresh review |
