...
| `cursor-agent (signed in)` ✗ (`--deep`) | `not signed in — run \`cursor-agent login\` on this host (it opens a browser)` | `cursor-agent login` **interactively on that host** — it opens a browser and blocks, so it can never be fixed unattended. Until it is, every Cursor step's pane comes up unusable |
...
| `ocr` ✗ | opaque `command -v` failure | Install `ocr` (pr-review's third review track shells out to it; nothing in the install puts it there). Only checked when a prompt names the `pr-review` skill — otherwise `not configured` |
