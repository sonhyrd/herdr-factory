...
| Parked `layout_wait_timeout` | budget spent | Read the reason's `— layout hook: …` suffix first (`explain` leads with it) — e.g. `no factory repo config for … (repo "<r>" config failed to load: …)` means fix the config. Then fix the layout and `resume` (refunds the 3 credits) |
