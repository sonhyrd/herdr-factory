...
Every invocation past gate 2 also writes its outcome line (the skip reason, `built "<id>"`, or the thrown error) to `<state>/layout-hook/last/<workspaceId>` (cleared with `decided/` at `[[startup]]`) — a `layout_wait_timeout` park quotes it.
...
| credits exhausted | parks: reason `layout_wait_timeout`, attention `<step>: layout pane <t>/<p> never became available` + ` — layout hook: <last line>` when the hook recorded one for the workspace (also `detail.hookLine`; `explain` leads with it), body `<step> step (belt <belt>): configured pane <t>/<p> didn't come up with an idle agent within <N>min (3 automatic retries exhausted) — is the herdr layout for this worktree running?` |
