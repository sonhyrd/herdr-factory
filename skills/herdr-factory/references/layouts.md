...
A **failed** build — the gate-5 config-load skip, or a thrown error — writes its line to `<state>/layout-hook/failed/<workspaceId>`; every other outcome (a build, a deliberate skip) removes it, and the `[[startup]]` hook clears the dir with `decided/`. A `layout_wait_timeout` park quotes only this failure line, so a layout that was built never takes the blame.
...
| waited past `min(120s, layout_wait_seconds)`, pane `unknown` with **no** agent label | the agent is running but herdr never detected it (e.g. `cursor-agent` after a self-update), so no prompt is confirmed. **Once per pane per step** (refunded on dispatch and `resume`): `/quit` in the pane (skipped if it is already at a shell prompt), wait for the shell, type the layout's command again (`cursor-agent` for kind `cursor`, else the kind, plus `args`), and poll up to 60s for detection. Logs `agent in pane <id> was never detected by Herdr (status unknown) — relaunching <cmd>`, records `pane_relaunched` `{step, tab, pane, paneId, command, detected}`, and re-arms the window **without** spending a credit. Never uses `agent start`, because herdr's stale registration refuses it. |
| window expires, credits left (limit **3**) | if the workspace has **none** of the (pruned) layout's tab labels — the hook built nothing, e.g. it ran while the repo's config didn't load — first **builds the layout** itself from the server's config (`… has none of layout "<id>"'s tabs — building it`; drops the stale `applied/` claim first; an empty tab list is treated as herdr not answering and skipped). Otherwise it  if that pane resolves by its configured label and `pane at-shell-prompt` says it is sitting at a prompt with no agent — the same `agent start` the build issues, kind and args straight off the layout pane (`… is at a shell prompt with no agent — re-starting <kind>`), trust pre-answered, and **under the name the build gave that pane** — the layout's agent panes are numbered in build order (`claude-w1`, `claude-w1-2`, …, over the layout as the hook PRUNED it), and herdr refuses a name a live agent still holds, so a layout with several same-kind agent panes could otherwise only ever re-start its first. Then bumps the guard counter, **re-arms in place** with a **full fresh window**, records `layout_wait_retry` `{step, tab, pane, attempt, limit, agentRestarted, layoutRebuilt?}`, logs `… not up after <waited>s — re-arming the wait (retry <a>/<limit>)`. A pane that already has an agent (busy, mid-turn, or still starting) is never touched. If that `agent start` is refused `agent_name_taken` / `agent_pane_busy` (a stale herdr registration), the layout's command is typed into the pane instead. |
| credits exhausted | parks: reason `layout_wait_timeout`, attention `<step>: layout pane <t>/<p> never became available` — or `<step>: agent in pane <id> was never detected by Herdr (status unknown)` when the pane is still `unknown` with no agent label (`detail.undetectedPane`; `explain` then says the agent was never detected instead of blaming plugin links or tab names) — + ` — layout hook: <failure line>` only when the build failed for the workspace (also `detail.hookLine`; `explain` leads with it, pointing at `doctor` only for a config-load line), body `<step> step (belt <belt>): configured pane <t>/<p> didn't come up with an idle agent within <N>min (3 automatic retries exhausted) — is the herdr layout for this worktree running?` |
...
The counter is also refunded on a successful dispatch. Related: when a live layout pane dies and no replacement resolves, the reconciler logs `<KEY>: <step> replacement pane not available — handing the retry to the layout wait` — **the engine never recreates a dead layout pane**; the hook fires once per worktree. The one build the engine does itself is the never-built layout above (none of its tabs present).
...
| `workspace <id> is not a fresh 1-tab/1-pane workspace; skipping` | a HAND-created worktree that was reopened/arranged, or had a pane split before the hook ran (a factory run's worktree is built anyway, as new tabs) | remove + recreate the worktree, or hand-build the tab/pane with byte-identical titles |
...
| `no factory repo config for … (repo "<r>" config failed to load: …)` | that repo's `config.yml` doesn't load with the installed code (often a key a newer version added). The server pauses claims for it meanwhile (`config invalid: …` in "needs you" / `doctor`) | fix the file (or update the host). The workspace wasn't settled, so the next focus builds it; a waiting run's next window expiry builds it too — no `resume` needed |
