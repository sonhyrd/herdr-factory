...
- **Changing the brand or the host name mid-flight is safe.** Readers accept the configured brand AND the legacy `herdr-factory` one, and a host recognises claims it posted under its raw hostname before `machine.yml`'s `host_alias` renamed it — while the release it posts to free one keeps that hostname, so every other host pairs claim and release too. Roll either out host by host; two hosts never both own an item during the switch.
