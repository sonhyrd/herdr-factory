...
- `doctor` resolves `git`/`herdr`/`gh`/`claude`/`cursor-agent`/`ocr` against the **installed service's PATH** parsed out of the plist/unit, not your shell's PATH — so it can report `✗` for a tool your shell runs fine. The `herdr` check also runs `herdr --version` and enforces the floor in `herdr-plugin.toml` (`>= 0.7.5`), reporting `v<x> is too old — the factory needs >= 0.7.5 (run \`herdr update\`)`; `--deep` adds the running server's protocol compatibility (`v0.7.5, protocol 17`) and an **`orphaned dev servers`** row — TCP listeners whose cwd is inside `~/.herdr/worktrees` with no live, unparked run owning that worktree (a parked run's server was stopped when it parked), with their RSS (amber, report-only; it never kills, and it needs `lsof`). Full check-by-check remediation is in [troubleshooting.md](./troubleshooting.md).
- With `--repo`, the `you provide` group gains four rows driven by **that repo's loaded config**, none of which fail loudly on their own at run time: `cursor-agent` (presence; `--deep` → `cursor-agent (signed in)`, which confirms the binary is on PATH **before** interpreting `cursor-agent status` — a missing binary is a ✗ naming the install, never the login — and otherwise reports the account, quoting the CLI's own first line when it isn't signed in), `cursor models` (`--deep`: every Cursor `--model` id in the config — repo/belt/step `agent.flags` and every layout pane's `agent_args` — must appear in `cursor-agent models`; `--model` on a `claude` agent is skipped; ✓ `not checked — cursor-agent is not on PATH` when the binary is absent, so one root cause is never two ✗s), `agent skills` (`--deep`: every skill the loaded prompts name exists as `SKILL.md` under the root of the engine that runs the step — `~/.claude/skills/` for Claude; any of `~/.cursor/skills/`, `~/.claude/skills/` or `~/.agents/skills/` for Cursor, which reads all three), and `ocr` (presence, only when a prompt names the `pr-review` skill). Each reports `not configured — …` (a ✓) when the config doesn't ask for it, so a Cursor-less host is never failed. `--deep` runs the two `cursor-agent` invocations concurrently: ~2s added.
...
**`rework` — the OPERATOR's bounce.** `rework <key> <toStep> --note "…"` (or `--note-file`) is the
only run-scoped signal that is *not* rendered into any prompt: an agent has `bounce`, which the
belt's `canBounceTo` constrains and which is stamped with the issuing step + pass. A person is not
a step, so `rework` carries neither stamp. It runs the same engine path (`bounceStep`, in operator
mode): the running step stops, `<toStep>` opens a new pass with the note written to
`.memory/herdr-factory/feedback-<toStep>.md` and surfaced by the **Rework requested — READ THIS
FIRST** banner, its own pane is re-prompted, a **`rework`** event is recorded
(`{by: "operator", fromStep, toStep, pass, bounces}`) and the **`max_bounces` cap applies**. It
lands from `running`, `waiting_for_human` (the pending question is closed as moot) and a parked
`attention` run — including human-only parks an agent bounce is refused from — and may target the
step that is *currently running*; forward, `reviewing` and teardown are refused. Tell the operator
when **not** to use it: a pre-existing problem outside this ticket's scope is a new ticket, not a
rework of this run.
...
All the run-scoped signals (the five agent ones plus the operator's `rework`) go through the same `applySignal` engine function on both the server and the local path, so the two can't drift. **A rejected signal prints to stderr and exits 1** — the exit code is an agent's only feedback, and a rejection that exited 0 used to leave the agent believing it had finished while the run sat until its step budget expired. Success (including the idempotent `already recorded done`) still exits 0:

| signal | success output | rejection messages (stderr, **exit 1**) |
|---|---|---|
| `step-done` | `<key>: advanced <step> → <next step>` when the nudge moved the belt; `<key>: <step> recorded done — the dispatcher advances the belt on its next pass` when the run lock was busy; `<key>: step "<s>" is already recorded done — nothing to do` on an idempotent replay. (It used to print **nothing**, which is why agents polled `status` and slept to find out whether the belt had moved.) | `no active run` · `step "<s>" is not in belt "<b>"` · `"<s>" is not the run's active step ("<active>") — signal ignored` (not raised for the belt's PR-opening step while the run is in `reviewing`: `run.step` is null there and that signal is what lifts the ready-to-merge gate) · `stale step-done for pass N — the <step> step is on pass M; finish the current pass and run its own step-done command` · the tree guard's `the <step> step cannot finish on this tree — the worktree has uncommitted changes. …` followed by the diff stat (a read-only step on a dirty tree) · `the <step> step cannot finish yet — the branch changes no file matching requires_changes (<globs>). …` followed by what changed since the base (a step with `requires_changes`) (`already recorded done` is treated as success — it prints the replay line above and exits 0) |
...
| `rework` (OPERATOR, not an agent signal) | `<key>: reworked to <toStep>[ — <message>]` · cap hit → `<key>: bounce limit exceeded — escalated to attention` | `no active run` · `step "<toStep>" is not in belt "<b>"` · `the run is in phase "<phase>" — rework only applies while the belt is still running (running, waiting_for_human or attention)` · `<toStep> is not before <fromStep> — rework goes backward (or re-runs the running step)` · `run busy — retry rework in a moment` |
...
Flag errors also exit 1: `rework: pass either --note or --note-file, not both` · `rework: provide a non-empty --note or --note-file (what the step must do differently)` · `ask-human: pass either --question or --question-file, not both` · `ask-human: provide a non-empty --question or --question-file` · `bounce: pass either --reason or --reason-file, not both` · `bounce: provide a non-empty --reason or --reason-file (the findings the earlier step must address)`.
...
- `server-first` commands (`tick`, `claim`, `teardown`, `resume`, `retry-now`, `rework`, `step-done`, `ask-human`, `bounce`, `capture-attempt`, `set-branch`) POST to `127.0.0.1:<port>` with a 10-minute timeout (they do real work).
...
- **One open per machine, and a forward that recovers on its own.** The endpoint is resolved per call and the TUI reads every repo concurrently, so concurrent reads of one machine share a single in-flight open — one `ssh -N`, one port, no `ControlPath` race and no burst of short-lived forwards per poll. A forward is never replaced or dropped without being torn down (no orphaned `ssh -N` left behind), and a held forward is re-probed with `/health` before it is handed out: a ControlMaster that outlived the remote server's restart is retired, the forward is re-opened, and the machine comes back **without restarting the TUI**. That probe crosses the forward, so it carries the same **2500ms** floor as every other read over one — a probe stricter than the read it guards would retire a healthy-but-distant machine's forward every poll and fork a replacement.
...
2. `capture-attempt` and `set-branch` are real commands with prompt tokens; `rework` is a real command with **no** token (it is the operator's, not an agent's).
