...
- With `--repo`, the `you provide` group gains four rows driven by **that repo's loaded config**, none of which fail loudly on their own at run time: `cursor-agent` (presence; `--deep` → `cursor-agent (signed in)`, which runs `cursor-agent status` and reports the account), `cursor models` (`--deep`: every Cursor `--model` id in the config — repo/belt/step `agent.flags` and every layout pane's `agent_args` — must appear in `cursor-agent models`; `--model` on a `claude` agent is skipped), `agent skills` (`--deep`: every skill the loaded prompts name exists as `SKILL.md` under `~/.cursor/skills/` or `~/.claude/skills/`, picked per the engine that runs the step), and `ocr` (presence, only when a prompt names the `pr-review` skill). Each reports `not configured — …` (a ✓) when the config doesn't ask for it, so a Cursor-less host is never failed. `--deep` runs the two `cursor-agent` invocations concurrently: ~2s added.
