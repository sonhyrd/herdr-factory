...
4. `## This is a read-only step (no commits)` — **only** for a `custom` step with `read_only: true`. `evidence`/`review` describe their own posture in their base prompts, so the note is suppressed there. Names the enforcement: HEAD movement during the step parks the run as a read-only violation, and the step's `step-done` is refused (with the diff stat) while the worktree is dirty or HEAD has moved under it.
