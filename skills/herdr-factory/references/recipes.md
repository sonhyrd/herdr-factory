...
Notes: claiming **consumes** `hf-review` (re-add it to re-review) and swaps in `herdr:in-development`, then `herdr:in-review`; a failed run leaves the PR open wearing `herdr:aborted`. `close_on` is ignored for a PR — the factory never closes or merges one. No `pr` step here (there is already a PR); the review's findings land wherever your `review` prompt posts them. Have that prompt post them as **one PR review** whose body starts with `<brand>-review-verdict: changes-requested|clean`, `<brand>-review-round: N`, `<brand>-review-head: <reviewed sha>` and lists the findings as a numbered checklist (`- [ ] 1. **must-fix** …`, then `**should-fix**`, `**nit**`): when the PR was built by a factory run that is still watching it and the review was posted by the factory's own GitHub login, that run picks the verdict up, fixes it and posts the next round itself (see the README's reviewing watch). A source is issues **or** PRs, never both: keep your issue belt on a second `github_issues` source over the same repo with its own `name`.
